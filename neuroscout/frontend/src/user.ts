...
export const setLocal = (user: Partial<UserStore>) => {
  ...
  localProfileItems
    .filter(x => !!user.profile && x in user.profile)
    .map(x => {
      localStorage.setItem(x, user.profile![x])
    })
}
...
export class UserStore {
  constructor(setState) {
    ...
    this.profile = {
      ...profileInit(),
      update: (updates: Partial<ProfileState>, updateLocal = false) => {
  ...
  update = (data: any, updateLocal = false) => {
    if (data) {
  ...
  authenticate = (): Promise<string> => {
    return new Promise((resolve, reject) => {
      const { password, isGAuth, accountconfirmError } = this
      const { email, name, picture } = this.profile
      fetch(domainRoot + '/api/auth', {
        method: 'post',
        body: JSON.stringify({
          email: isGAuth === true ? 'GOOGLE' : email,
          password: password,
        }),
        headers: {
              ...
              fetch(domainRoot + '/api/user', {
                ...
                .then(response => {
                  if (response.status === 401) {
                    void response
                      .json()
                      .then((missing_data: { message: string }) => {
                        if (
                    ...
                    void message.success('Logged in.')

                    localStorage.setItem('jwt', token)

                    void response.json().then((user_data: ApiUser) => {
                      const profileItems: Partial<ProfileState> = {}
                      localProfileItems.map(
                        x => (profileItems[x] = user_data[x]),
                      )
  ...
  loadPredictorCollections = (user?: ApiUser) => {
    void api.getPredictorCollections().then(colls => {
  ...
  signup = (newUser: { password: string; name: string; email: string }) => {
    ...
    fetch(domainRoot + '/api/user', {
      ...
      .then(response =>
        response
          .json()
          .then(json => ({ ...json, statusCode: response.status })),
      )
      .then((data: any) => {
  ...
  accountconfirmError = (jwt): void => {
    Modal.confirm({
      ...
      cancelText: 'Close',
      onOk() {
        fetch(String(domainRoot) + '/api/user/resend_confirmation', {
          method: 'post',
          headers: {
            'Content-type': 'application/json',
            authorization: 'JWT ' + String(jwt),
          },
        }).catch(displayError)
      },
    })
  ...
  resetPassword = (): void => {
    ...
    void fetch(String(domainRoot) + '/api/user/reset_password', {
      method: 'post',
        ...
        if (response.ok) {
          ...
          void response
            ...
            .then((data: any) => {
              ...
              update({ resetError: errorMessage })
            })
        }
      })
      .catch(displayError)
