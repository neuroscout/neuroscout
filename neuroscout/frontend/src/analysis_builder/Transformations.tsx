        ...
        return (
            <div>
                ...
                <PredictorSelector
                    ...
                    updateSelection={(predictors) =>
                        onChange(predictors.map((x) => x.name))
                    }
                    compact
                />
                <br />
              ...
              <Row key={i}>
                  <div>
                      ...
                      <Col span={4}>
                          <InputNumber
                              onChange={(newValue) => {
                                  const newWeights = this.updateWeight(i, newValue as number);
                                  onChange(newWeights);
                              }}
                              defaultValue={
...
class XformEditor extends React.Component<XformEditorProps, XformEditorState> {
    ...
    // eslint-disable-next-line @typescript-eslint/no-shadow
        ...
        const availableParameters = name
            ? Object.keys(xformRules[name])
            : undefined;
        return (
            <div>
                ...
                availableParameters.map((param) => {
                    ...
                    // that were selected for 'inputs'
                        const inputIds = new Set(input.map((x) => x.name));
                        options = availableInputs.filter(
                            (x) => !inputIds.has(x.name)
                        );
                    }
                    return (
                        <div key={param}>
