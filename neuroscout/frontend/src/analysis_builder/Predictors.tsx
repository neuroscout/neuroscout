...
export class PredictorReview extends React.Component<PredictorReviewProps, Record<string, never>> {
    render() {
        const columns = [
            {
                title: 'Name',
...
> {
    ...
    updateFilters = (predictors) => {
        ...
        this.filterFields.map((field) => {
            ...
            const updatedFilters = [...unique].sort().map((x) => {
    ...
    sourceCmp = (a, b) => {
        const x = a.source + a.name;
        const y = b.source + b.name;
    ...
    filterCheckboxes = (filterType) =>
        this.state[filterType + 'Filters'].map((filter) => {
            const display =
