...
{ traceback: string; message: string },
{}
> {
    render() {
        const display: any[] = [];
...
export class Report extends React.Component<ReportProps, ReportState> {
    ...
    generateReport = (scale?: boolean): void => {
        ...
        const url = `${domainRoot}/api/analyses/${id}/report?run_id=${this.state.selectedRunIds}${scale_param}`;
        jwtFetch(url, { method: 'POST' }).then((res) => {
    ...
    loadReports = () => {
        ...
        jwtFetch(
            `${domainRoot}/api/analyses/${id}/report?run_id=${this.state.selectedRunIds}`
        ).then((res) => {
            if (res.warnings) {
    ...
    componentDidMount() {
        ...
        if (this.state.compileLoaded === false) {
            const id = this.props.analysisId;
            const state = { ...this.state };
            jwtFetch(`${domainRoot}/api/analyses/${id}/compile`).then((res) => {
